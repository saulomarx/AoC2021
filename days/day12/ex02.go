...
func walk(nodeMap map[string]*node, currentNodeValue, path string) string {
	...
	if value == "end" {
		...
		for _, neighborValue := range currentNode.neighbors {
			if nodeMap[neighborValue].nodeType == "big" {
				newPath := path + currentNodeValue + ","
				walk(nodeMap, neighborValue, newPath)
			} else if nodeMap[neighborValue].nodeType == "small" {
				visits := strings.Count(path, neighborValue)
				if visits == 0 {
					newPath := path + currentNodeValue + ","
					walk(nodeMap, neighborValue, newPath)
				}
				if visits == 1 && !strings.Contains(path, "2") {
					newPath := path + currentNodeValue + ",2,"
					walk(nodeMap, neighborValue, newPath)
				}
			} else if neighborValue != "start" {
				newPath := path + currentNodeValue + ","
				walk(nodeMap, neighborValue, newPath)
			}
		}
