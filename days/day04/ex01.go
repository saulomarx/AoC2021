...
func createBoard(board [5][5]int, sum int) *bingoBoard {
	b := bingoBoard{board: board, resultBoard: matrix, sum: sum}
	return &b
}
...
func main() {
	...
	playBingo(numbers, numberToMap, boardMap)
}

func playBingo(numbers []int, numberToMap map[int][]int, boardMap map[int]*bingoBoard) {
